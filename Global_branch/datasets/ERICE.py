...
class ERICE:
    ...
    def get_loaders(self):
        ...
        train_dataset = ERICEDataset(dir=os.path.join(self.config.data.data_dir, 'Train'), val=False, transforms=self.transforms)
        val_dataset = ERICEDataset(dir=os.path.join(self.config.data.data_dir, 'Test'), val=True, transforms=self.transforms)
...
class ERICEDataset(torch.utils.data.Dataset):
    def __init__(self, dir, val,  transforms):
        ...
        Diff_output = os.path.join(ERICE_dir, 'Diffusion-branch/results/images/ERICE')
        ...
        gt_names += [os.path.join(os.path.join(ERICE_dir, 'GT'), i) for i in images]
