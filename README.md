...
Then, you can run the following script to train the global branch: 
```
cd Global_branch
python train_diffusion.py --config 'DHID.yml' --image_folder './results'
```
Please note that the global branch requires the output images of diffusion branch as its input images. By default, the weight files will be saved in `checkpoints/`.
...
```
Then, you can load the ckpt generated by training the global branch to evaluate the performance of the global branch.
```
cd Global_branch
python eval_diffusion.py --config "DHID.yml" --resume 'DHID.pth.tar'
```
