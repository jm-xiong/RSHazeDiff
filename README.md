...
Then, you use the output images of the diffusion branch as one input and load the ckpt generated by training the global branch to evaluate the performance.
```
cd Global_branch
python test.py --config "DHID.yml" --resume 'DHID.pth.tar'
```
Please note that the output of the global branch is final output. 
