...
class DHID:
    ...
    def get_loaders(self, parse_patches=True):
        print("=> evaluating DHID set...")
        train_dataset = DHIDDataset(dir=os.path.join(self.config.data.data_dir, 'TrainingSet'),
        ...
        val_dataset = DHIDDataset(dir=os.path.join(self.config.data.data_dir, 'TestingSet', 'Test'),
                                         n=self.config.training.patch_n,
                                         patch_size=self.config.data.image_size,
                                         transforms=self.transforms,
                                         parse_patches=parse_patches)
...
class DHIDDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, val=True, filelist=None, parse_patches=True):
        ...
        if filelist is None:
            ...
            DHID_inputs = os.path.join(DHID_dir, 'Haze')
            images = [f for f in sorted(listdir(DHID_inputs)) if isfile(os.path.join(DHID_inputs, f))]

            input_names += [os.path.join(DHID_inputs, i) for i in images]
            gt_names += [os.path.join(os.path.join(DHID_dir, 'GT'), i.split('_')[0] + '.jpg') for i in images]
