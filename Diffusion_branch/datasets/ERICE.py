...
class ERICE:
    ...
    def get_loaders(self, parse_patches=True, validation='rainfog'):
        print("=> evaluating ERICE set...")
        train_dataset = ERICEDataset(dir=os.path.join(self.config.data.data_dir, 'Train'),
                                           n=self.config.training.patch_n,
                                           patch_size=self.config.data.image_size,
                                           transforms=self.transforms,
                                           val=False,
                                           parse_patches=parse_patches)
        val_dataset = ERICEDataset(dir=os.path.join(self.config.data.data_dir, 'Test'),
                                         n=self.config.training.patch_n,
                                         patch_size=self.config.data.image_size,
                                         transforms=self.transforms,
                                         parse_patches=parse_patches)
...
class ERICEDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, val=True, filelist=None, parse_patches=True):
        super().__init__()
        ...
        if filelist is None:
            ...
            # ERICE-Rain train filelist
            ERICE_inputs = os.path.join(ERICE_dir, 'Haze')

            images = [f for f in sorted(listdir(ERICE_inputs)) if isfile(os.path.join(ERICE_inputs, f))]
            input_names += [os.path.join(ERICE_inputs, i) for i in images]

            ERICE_gt = os.path.join(ERICE_dir, 'GT')
            gt_names += [os.path.join(ERICE_gt, i) for i in sorted(listdir(ERICE_gt))]
