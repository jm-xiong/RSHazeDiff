...
class LHID:
    ...
    def get_loaders(self, parse_patches=True):
        ...
        train_dataset = Dataset(dir=os.path.join(self.config.data.data_dir, 'TrainingSet'),
        ...
        val_dataset = Dataset(dir=os.path.join(self.config.data.data_dir, 'TestingSet', 'Merge'),
                                         n=self.config.training.patch_n,
                                         patch_size=self.config.data.image_size,
                                         transforms=self.transforms,
                                         parse_patches=parse_patches)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, val=True, filelist=None, parse_patches=True):
        ...
        if filelist is None:
            ...
            # -Rain train filelist
            _inputs = os.path.join(_dir, 'Haze')
            images = [f for f in sorted(listdir(_inputs)) if isfile(os.path.join(_inputs, f))]
            input_names += [os.path.join(_inputs, i) for i in images]

            gt_names += [os.path.join(os.path.join(_dir, 'GT'), i.split('_')[0] + '.jpg') for i in images]
